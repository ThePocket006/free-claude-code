...
def test_settings_defaults_are_valid_and_nonempty() -> None:
    ...
    # Fields that intentionally default to empty string (per-tier fallbacks).
    empty_ok = {
    ...
    assert [
        ...
        if isinstance(value, str) and not value and name not in empty_ok
    ] == []
    assert [
        name
        for name, field in Settings.model_fields.items()
        if field.get_default(call_default_factory=True) == "" and name not in empty_ok
    ] == []
...
def test_model_fallbacks_parses_comma_string_to_tuple() -> None:
    settings = Settings.model_validate(
        {
            "MODEL_FALLBACKS": "open_router/anthropic/claude-sonnet-4, gemini/models/gemini"
        }
    )
    assert settings.model_fallbacks == (
...
def test_model_fallbacks_default_is_none() -> None:
    assert Settings().model_fallbacks is None


def test_model_fallbacks_invalid_provider_raises() -> None:
    with pytest.raises(ValidationError, match="Invalid provider"):
        Settings.model_validate({"MODEL_FALLBACKS": "bogus_provider/some-model"})


def test_model_fallbacks_no_slash_raises() -> None:
    with pytest.raises(ValidationError, match="prefixed with provider type"):
        Settings.model_validate({"MODEL_FALLBACKS": "noprefix"})


def test_per_tier_fallback_fields_parse_and_validate() -> None:
    settings = Settings.model_validate(
        {
            "MODEL_FALLBACKS_OPUS": "deepseek/deepseek-chat",
            "MODEL_FALLBACKS_HAIKU": "gemini/models/gemini",
        }
    )
    assert settings.model_fallbacks_opus == "deepseek/deepseek-chat"
    assert settings.model_fallbacks_haiku == "gemini/models/gemini"


def test_per_tier_fallback_invalid_provider_raises() -> None:
    with pytest.raises(ValidationError, match="Invalid provider"):
        Settings.model_validate({"MODEL_FALLBACKS_SONNET": "bogus_provider/model"})


def test_provider_api_keys_parses_json_string() -> None:
    settings = Settings.model_validate(
        {"provider_api_keys": '{"nvidia_nim": "k1,k2,k3", "open_router": "ka,kb"}'}
    )
...
def test_provider_api_keys_defaults_to_empty() -> None:
    assert Settings().provider_api_keys == {}


def test_provider_api_keys_non_json_raises() -> None:
    with pytest.raises(ValidationError, match=r"(?i)provider_api_keys"):
        Settings.model_validate({"provider_api_keys": "nvidia_nim=k1,k2"})


def test_resilience_settings_defaults() -> None:
    settings = Settings()
    assert settings.provider_key_cooldown_seconds == 60.0
    assert settings.circuit_breaker_threshold == 3
    assert settings.circuit_breaker_cooldown == 120.0
